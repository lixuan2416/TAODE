...
std::string getStoreId(StoreInst *SI) {
	...
	Anno = getAnnotation(V, M);				// getAnnotation —— 分析 V 指针的来源, 返回其命名
	if (Anno.empty())
		return Anno;
...
// getAnnotation —— 分析 V 指针的来源, 返回其命名
...
// (4) ConstantExpr            constant GEP expression
			if (CE->getOpcode() == Instruction::GetElementPtr) {
				is = CE->op_begin() + 1;
				ie = CE->op_end() - 1;
				PVal = CE->getOperand(0);
...
// (5)  id is in the form of struct.[name].[offset]
		if (PVal) {
